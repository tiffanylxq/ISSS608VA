...
In this take-home exercise, we used appropriate static statistical graphics methods to reveal the demographic of the city of    Engagement, Ohio USA from the [VAST Challenge 2022](https://vast-challenge.github.io/2022/). Demographic is the statistical characteristics of a given population such as age, gender, ethnicity, income, level of education, religion, occupation, family structure etc.

About 1000 representative residents has volunteered to provide data using the city’s urban planning app, which records the places they visit, their spending, and their purchases, among other things. 
...
The packages required are **tidyverse** (including **dplyr**, **magrittr**, **ggplot2**, **patchwork**), **skimr**, **ggrepel** will be used for the purpose of this exercise:
...
### Importing Data
There are a total of 3 dataset folders (**Activity Logs, Attributes, Journals**) from the VAST Challenge 2022. We will only be using the dataset *Participants* from the *Attributes* folder for this exercise. 
...
First, let’s just get a general sense of our data using the function `glimpse()` from the library `dplyr`.
...
```
We see that participants are quite well distributed across all the age groups with a dip in the late 30s range. 
...
ggplot(data=demographic_data, aes(x = householdSize)) +
  ...
  xlab("Age") + 
  ylab("No. of Participants")+
  ggtitle(label = 'Household Size Distribution',
          subtitle = 'The household size ranges from 1 to 3.')+
    theme(plot.title = element_text(size=14, face="bold",hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5))
...
```  
The singles (i.e. househould size = 1) are mostly in their late 40s, and we see mostly young families (late 20s to late 30s) within among the participants.
...
Now, let us see what are the relationship of the joviality of the participants with the various attributes. We prepared four different charts to compare joviality with whether they have kids, their education level, age and interest group.
...
```
In general, it seems that there are not much distinct difference in the degree of joviality across all attributes i.e. everyone is as cheerful as one another. A closer look, perhaps participants who have kids or hold a graduate degree or have interest "E" are typical more jovial than their peers.  

Hence, we created a trellis plot with educational level, interest group, kids to see which combination may gives a more jovial individuals. For an easier comparison, an arbitrary line of 0.8 (joviality) is inserted across all panels. 
